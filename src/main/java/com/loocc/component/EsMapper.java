 ...
 */
@Component
public class EsMapper extends AbstractResultMapper {
    ...
    public EsMapper(MappingContext<? extends ElasticsearchPersistentEntity<?>, ElasticsearchPersistentProperty> mappingContext) {
        super(new DefaultEntityMapper(mappingContext));
        Assert.notNull(mappingContext, "MappingContext must not be null!");
        this.mappingContext = mappingContext;
    }

    public EsMapper(EntityMapper entityMapper) {
        this(new SimpleElasticsearchMappingContext(), entityMapper);
    }

    public EsMapper(MappingContext<? extends ElasticsearchPersistentEntity<?>, ElasticsearchPersistentProperty> mappingContext, EntityMapper entityMapper) {
        super(entityMapper);
        Assert.notNull(mappingContext, "MappingContext must not be null!");
        this.mappingContext = mappingContext;
    }
        ...
        while(var8.hasNext()) {
            SearchHit hit = (SearchHit)var8.next();
                ...
                if (!StringUtils.isEmpty(hit.getSourceAsString())) {
                    ...
                    result = this.mapEntity(hit.getSourceAsString(), clazz);//把hit.getSourceAsString转换成对象
                } else {
                    /**
                     * 若我们设置了addFields()方法，但是并没有采用hit.getFields()方式，
                     * 而直接使用hit.getSource()或hit.getSourceAsString()方法获取数据，所有的数据将会是null。
                     */
